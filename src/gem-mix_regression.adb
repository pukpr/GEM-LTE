...
function GEM.Mix_Regression (Target : in LTE.Primitives.Data_Pairs;
                             ...
                             Display_Results : in Boolean := False) return LTE.Primitives.Data_Pairs is
   ...
   procedure Fit(P : in Gem.LTE.Long_Periods;
                 A : in out Gem.LTE.Long_Periods_Amp_Phase) is
    --             Start, Finish : in Long_Float;
   ...
   function Ex(A : in Gem.LTE.Long_Periods_Amp_Phase) return Gem.LTE.Long_Periods_Amp_Phase is
   begin
      if Exclude then
         return A(0..-1);
      else
