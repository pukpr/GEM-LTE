...
package body GEM.LTE.Primitives.Solution is
   ...
   Monotonic_Increase : constant Boolean :=  GEM.Getenv("SECULAR", TRUE); -- FALSE
   Trigger : LONG_FLOAT :=  GEM.Getenv("TRIGGER", 0.999999);
   ...
   Trop_Mod  : constant Boolean :=  GEM.Getenv("TROPMOD", FALSE);
   Delay_Difference : constant LONG_FLOAT:=  GEM.Getenv("DD", 0.0); -- 1.0 = Full
   ...
   procedure Dipole_Model (N_Tides, N_Modulations : in Integer;
      ...
      ImpA : constant Long_Float := GEM.Getenv("IMPA", -1.0); -- Defaults for ENSO
      ImpB : constant Integer := GEM.Getenv("IMPB", -1);
      ...
      DTW_Window : constant Integer :=  GEM.Getenv("DTW", 1); -- 100 is about ~5% of the time-series length
      ...
      begin
         if RMS_Metric then
            ...
            return Ada.Numerics.Long_Elementary_Functions.SQRT(DTW_Distance(X,Y, DTW_Window ) * CC(X,Y)); 
         elsif CTW_Metric then
            return Ada.Numerics.Long_Elementary_Functions.SQRT(long_float'Max(DTW_Distance(X,Y, DTW_Window),0.0) * CID(X,Y)) ; 
         elsif MLR_On then
      ...
      begin
         Value := COS(4.0*Pi*(Time+D.B.ImpB)); --4.0
         if Sin_Power = 1 then
   ...
   function Annual_Add (Model : in Data_Pairs; 
                        Polarity : Long_Float := 1.0) return Data_Pairs is
      M : Data_Pairs := Model;
      Pi : Long_Float := Ada.Numerics.Pi;
      aTwoPi : Long_Float := D.B.IR *2.0*Pi;
      use Ada.Numerics.Long_Elementary_Functions;
   begin
      for I in Model'Range loop
            M(I).Value := M(I).Value + Polarity*D.B.ImpA*Cos(aTwoPi*M(I).Date + D.B.ImpB)
                                     + Polarity*D.B.Ann1*Cos(2.0*pi*M(I).Date + D.B.Ann2)
                                     + Polarity*D.B.Sem1*Cos(4.0*pi*M(I).Date + D.B.Sem2); 
            -- + Polarity*D.B.ImpA*Cos(0.5*X*M(I).Date + D.B.ImpB);
      end loop;
      return M;
   end;
      ...
      loop
         ...
         Impulses := Impulse_Amplify(
                            Raw     => Tide_Sum(Template     => Data_Records,
                                                ...
                                                Periods      => D.A.LP,
                                                Ref_Time     => D.B.IR, -- D.B.ShiftT,
                                                Scaling      => 0.0, -- D.B.Offset,
         ...
         Forcing := IIR( Raw => Impulses,
                         ...
                         iA => D.B.init, iB => 0.0, iC => D.B.ImpC); -- D.B.Offset

         
         if LTE_Abs > 0.0 then
            for I in Forcing'Range loop
               if LTE_Abs = 1.0 then
                  Forcing(I).Value := abs Forcing(I).Value; -- + (1.0-LTE_Abs) * Forcing(I).Value;
               else
                  Forcing(I).Value := (abs Forcing(I).Value) ** LTE_Abs; -- + (1.0-LTE_Abs) * Forcing(I).Value;
               end if;
            end loop;
...
--               Average := 0.0;
--               for I in Model'Range loop
--                  Average := Average + abs Model(I).Value; -- + (1.0-LTE_Abs) * Forcing(I).Value;
--               end loop;
--               Average := Average / Long_Float(Model'Length);
--               for I in Model'Range loop
--                  Model(I).Value := abs Model(I).Value - Average; -- + (1.0-LTE_Abs) * Forcing(I).Value;
--               end loop;
--            end if;
         else
            -- Forcing := Median(Forcing);
         ...
         if Diff_Delay_N > 0 then -- Delay_Difference > 0.0 then
            for I in reverse Model'First + Diff_Delay_N .. Model'Last loop
               if D.B.Offset /= 0.0 then
                  Model(I).Value := Model(I).Value - D.B.Offset * Model(I-Diff_Delay_N).Value; -- - (1.0-D.B.Offset) * Model(I-Diff_Delay_N/2).Value;
               else
                  Model(I).Value := Model(I).Value - Delay_Difference * Model(I-Diff_Delay_N).Value;
               end if;
            end loop;
         end if;
