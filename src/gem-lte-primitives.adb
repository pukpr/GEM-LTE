...
package body GEM.LTE.Primitives is
   ...
   function LTE (Forcing : in Data_Pairs;
                 ...
                 Third : in Long_Float := 0.0) return Data_Pairs is
  ...
  function CID(X, Y : in Data_Pairs) return Long_Float is
   ...
   begin
      ...
      return Sqrt(Long_Float'Min(Sum_X, Sum_Y) / Long_Float'Max(Sum_X, Sum_Y));
   end CID;
   ...
   begin
      for I in First .. Last loop
         ...
         for K in DBLT'First .. NM loop  -- D.B.LT'First = 1
            Value := Sin(2.0*Pi*DBLT(K) * Forcing(I).Value) * exp(Forcing(I).Value * Third);
            Factors_Matrix(I-First+1, 3+(K-1)*2) := Value;
            Value := Cos(2.0*Pi*DBLT(K) * Forcing(I).Value) * exp(Forcing(I).Value * Third);
