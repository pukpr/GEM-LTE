...
REL_EPS = 0.0001  # 0.01% relative tolerance for grouping keys
...
def rel_close(a: float, b: float, rel_eps: float = REL_EPS) -> bool:
    if a == b:
        return True
...
def assign_group(groups: List[Group], key: float) -> int:
    for i, g in enumerate(groups):
        if rel_close(g.rep, key):
            g.keys.append(key)
            # update representative to running mean of observed keys
            g.rep = (g.rep * (len(g.keys) - 1) + key) / len(g.keys)
            return i
...
def make_scatter_with_errorbars(groups: List[Group], out_png: str) -> None:
    ...
    for i, g in enumerate(groups):
        ...
        y_means.append(m)
        y_sds.append(sd)
        x_labels.append(f"{g.rep:.6g}")

    plt.figure(figsize=(max(6, 0.4 * len(x_labels)), 8))

    x_jitter = 0.25  # width of horizontal spread (tune)

    xs_j = [x + (random.random() - 0.5) * x_jitter for x in xs_all]
    plt.plot(xs_j, ys_all, linestyle="None", marker="_", markersize=1)
...
def main() -> None:
    ...
    ap.add_argument("--plot", default="scatter_by_key.png",
    ...
    print(f"# Relative epsilon for grouping keys: {REL_EPS*100:.4f}%")
    print()
    ...
    print("-" * 70)
    for r in stats_rows:
        print(f"{r['key_rep']:14.6g}  {r['n']:7d}  {r['mean']:14.6g}  {r['stdev']:14.6g}  {r['stdev_pct_mean']:11.4g}")
    ...
    if not outliers:
        ...
        for o in outliers:
            print(f"{abs(o['z']):8.3f}  {o['value']:14.6g}  {o['mean']:14.6g}  {o['stdev']:14.6g}  "
                  f"{o['key_rep']:14.6g}  {o['key_raw']:14.6g}  {o['file']}")
